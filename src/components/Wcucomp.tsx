...
const Wcucomp = ({ image, header, desc }: wcucompProps) => {
  return (
    ...
    >
      <img src={image} width={"150px"} height={"150px"} className=" mx-auto " />
      <motion.h1 className=" text-[2vh]  text-white text-center group-hover:text-green-400  transition-all duration-300">
        {header}
      </motion.h1>
      <motion.p className=" group-hover:text-green-200  transition-all duration-300 text-center text-gray-400 text-sm w-full">
        {desc}
      </motion.p>
    </motion.div>
  );
